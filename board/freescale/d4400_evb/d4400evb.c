...
static void setup_serdes_sgmii_mode(void)
{
	...
	/* Clear RSTREQ of SERDES PLL1 reset control */
	serdes_regs->pll1_rstctl_offs &= 0x7fffffff;

	/* Set RSTREQ of SERDES PLL1 reset control */
	serdes_regs->pll1_rstctl_offs |= 0x80000000;

	/* RST_DONE should be 1, RST_ERR should be 0 */
	mask = (serdes_regs->pll1_rstctl_offs & 0x60000000);
...
static void setup_enet(void)
{
	...
	if (BOOT_ETH_MODE_SGMII == d4400_get_eth_mode())
