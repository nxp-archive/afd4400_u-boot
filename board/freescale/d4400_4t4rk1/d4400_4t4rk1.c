...
#define UART3_RS485_DIR_PAD_CTRL    (PAD_CTL_PKE |		\
	PAD_CTL_PUS_100K_DOWN | PAD_CTL_SPEED_LOW |		\
	PAD_CTL_DSL_1 | PAD_CTL_HYS)

#define RX_8V_EN_PAD_CTRL     (PAD_CTL_PKE |	\
	PAD_CTL_PUS_100K_DOWN | PAD_CTL_SPEED_LOW |		\
	PAD_CTL_DSL_1 | PAD_CTL_HYS)

#define RX_8V_PGOOD_PAD_CTRL    (PAD_CTL_PKE | PAD_CTL_PUE |	\
	PAD_CTL_PUS_100K_UP | PAD_CTL_SPEED_MED |		\
	PAD_CTL_DSL_1 | PAD_CTL_HYS | PAD_CTL_ODE)
...
static struct ipmi_info ipmi_4t4rk1;

static const char _4t4rk1_rev_letter[6] = {
	'A', 'B', 'C', 'D', 'E', 'F',
};
...
static void get_board_info(void)
{
	...
	if (!board_checked) {
		...
		board_checked = 1;
		brd_type = BOARD_TYPE_D4400_4T4RK1;
		brd_rev  = BOARD_REV_A;
		...
		while (addrlen <= CONFIG_SYS_I2C_EEPROM_ADDRLEN_MAX) {
			...
			ret = ipmi_create(buf, &ipmi_4t4rk1);
			if (!ret)
				break;
			else
				++addrlen;
		...
		if ((!ret) && (addrlen <= CONFIG_SYS_I2C_EEPROM_ADDRLEN_MAX)) {
			brd_type = ipmi_get_board_type(ipmi_4t4rk1.board.name_str);
			brd_rev = ipmi_get_board_rev(ipmi_4t4rk1.board.partnum_str);
...
iomux_cfg_t uart_pads[] = {
#if (CONFIG_D4400_UART_PORT == 4) // hht, POC 4t4rk1
	D4400_PAD_UART4_TXD_UART4_TXD | MUX_PAD_CTRL(UART_PAD_CTRL),
	D4400_PAD_UART4_RXD_UART4_RXD | MUX_PAD_CTRL(UART_PAD_CTRL),
	D4400_PAD_I2C11_SDA_UART4_RTS | MUX_PAD_CTRL(UART_PAD_CTRL), /* RTS output */
	D4400_PAD_I2C11_SCL_UART4_CTS | MUX_PAD_CTRL(UART_PAD_CTRL), /* CTS input */
#endif
#if (CONFIG_D4400_UART_PORT == 3) // hht, POC 4t4r
...
{
	d4400_iomux_setup_multiple_pads(uart_pads, ARRAY_SIZE(uart_pads));
}
#endif /* CONFIG_D4400_UART */
...
iomux_cfg_t spi_pads[] = {
	...
	/* PA connector 1 has been removed from 4t4rk1 board
...
// hht Don't know if this is needed
...
static struct d4400_i2c_bus d4400_i2c_busses[] = {
	/* I2c bus number are accessed via U-boot i2c utility as follows:
	 *
	 * U-boot => i2c dev 0 (for I2C1)
	 * U-boot => i2c dev 1 (for I2C2)
	 * U-boot => i2c dev 2 (for I2C6)
	 * U-boot => i2c dev 3 (for I2C7)
	 * U-boot => i2c dev 4 (for I2C9)
	 */
...
#ifdef CONFIG_4T4RK1_CONTROL
iomux_cfg_t gpio_uart3_rs485_dir_pads[] = {
	D4400_PAD_GPIO_B2 | MUX_PAD_CTRL(UART3_RS485_DIR_PAD_CTRL),
};

iomux_cfg_t gpio_led_pads[] = {
	D4400_PAD_GPIO_B4 | MUX_PAD_CTRL(LED_PAD_CTRL),
	D4400_PAD_GPIO_B5 | MUX_PAD_CTRL(LED_PAD_CTRL),
	D4400_PAD_GPIO_B18 | MUX_PAD_CTRL(LED_PAD_CTRL),
	D4400_PAD_GPIO_B19 | MUX_PAD_CTRL(LED_PAD_CTRL),
};

iomux_cfg_t gpio_rx_8v_ctrl_pads[] = {
	D4400_PAD_GPIO_B8 | MUX_PAD_CTRL(RX_8V_EN_PAD_CTRL),
	D4400_PAD_GPIO_B9 | MUX_PAD_CTRL(RX_8V_EN_PAD_CTRL),
	D4400_PAD_GPIO_B10 | MUX_PAD_CTRL(RX_8V_EN_PAD_CTRL),
	D4400_PAD_GPIO_B11 | MUX_PAD_CTRL(RX_8V_EN_PAD_CTRL),

	D4400_PAD_GPIO_B12 | MUX_PAD_CTRL(RX_8V_PGOOD_PAD_CTRL),
	D4400_PAD_GPIO_B13 | MUX_PAD_CTRL(RX_8V_PGOOD_PAD_CTRL),
	D4400_PAD_GPIO_B14 | MUX_PAD_CTRL(RX_8V_PGOOD_PAD_CTRL),
	D4400_PAD_GPIO_B15 | MUX_PAD_CTRL(RX_8V_PGOOD_PAD_CTRL),
};
...
static void setup_iomux_4t4rk1_gpio(void)
{
	...
	/* Uart3 RS-485 direction control is active low, set low to
	 * enable receiver.
	 */
	d4400_iomux_setup_multiple_pads(gpio_uart3_rs485_dir_pads,
		ARRAY_SIZE(gpio_uart3_rs485_dir_pads));
	gpio_direction_output(D4400_GPIO_NR(2, 2), 0); /* GpioB 2 */

	/* Board LED control (2), set low to turn on LED */
	d4400_iomux_setup_multiple_pads(gpio_led_pads,
		ARRAY_SIZE(gpio_led_pads));
	gpio_direction_output(D4400_GPIO_NR(2, 4), 0); /* GpioB 4 */
	gpio_direction_output(D4400_GPIO_NR(2, 5), 0); /* GpioB 5 */
	gpio_direction_output(D4400_GPIO_NR(2, 18), 0); /* GpioB 18 */
	gpio_direction_output(D4400_GPIO_NR(2, 19), 0); /* GpioB 19 */

	/* Rx path 8v enable control and powergood status */
	d4400_iomux_setup_multiple_pads(gpio_rx_8v_ctrl_pads,
		ARRAY_SIZE(gpio_rx_8v_ctrl_pads));

	/* Rx LNA 8v bias enable is active low, set high to turn OFF */
	gpio_direction_output(D4400_GPIO_NR(2, 8), 1); /* GpioB 8 */
	gpio_direction_output(D4400_GPIO_NR(2, 9), 1); /* GpioB 9 */
	gpio_direction_output(D4400_GPIO_NR(2, 10), 1); /* GpioB 10 */
	gpio_direction_output(D4400_GPIO_NR(2, 11), 1); /* GpioB 11 */
	/* Rx LNA 8v bias status input is active low, low indicates
	 * bias is ON.
	 */
	gpio_direction_input(D4400_GPIO_NR(2, 12)); /* GpioB 12 */
	gpio_direction_input(D4400_GPIO_NR(2, 13)); /* GpioB 13 */
	gpio_direction_input(D4400_GPIO_NR(2, 14)); /* GpioB 14 */
	gpio_direction_input(D4400_GPIO_NR(2, 15)); /* GpioB 15 */
}
#endif
...
int board_init(void)
{
	...
	if (BOARD_REV_UNKNOWN != rev)
		rev_letter = (type == BOARD_TYPE_D4400_4T4RK1) ?
					_4t4rk1_rev_letter[rev] : 'A' + rev;
	switch (get_board_type()) {
	case BOARD_TYPE_D4400_4T4RK1:
		printf("Board: D4400-4T4RK1, Rev %c\n", rev_letter);
		break;
	default:
		printf("Board: D4400-???\n");
		break;
